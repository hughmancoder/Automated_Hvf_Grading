from automated_hvf_grading_module import ParallelProcessing
import argparse
import pandas as pd
...
if args.files:
    ...
    ParallelProcesser = ParallelProcessing.ParallelProcess()
    data = ParallelProcesser.runParallel()
    ...
    htmloutput = df.to_html(border=0, table_id="grading-table", index=False, justify="left").replace('\n', ' ').replace('\r', '')
    ...
    if args.progression == 0:
        ...
        htmloutput = outdf.to_html(border=0, table_id="analysis-table", index=False, justify="left").replace('\n', ' ').replace('\r', '')
