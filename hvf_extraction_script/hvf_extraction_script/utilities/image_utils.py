...
class Image_Utils:
    ...
    def crop_white_border(image):
        ...
        for row_index in range(0, np.size(image, 0)):
            # All white row
            if reduce((lambda x, y: x and y), element_mask[row_index, :]) == True:
                y0 = row_index
                continue
        ...
        for col_index in range(0, np.size(image, 1)):
            # All white row
            if reduce((lambda x, y: x and y), element_mask[:, col_index]) == True:
                x0 = col_index
                continue
            else:  # We have at least 1 black pixel - stop
                break
    ...
    def delete_stray_marks(image, global_threshold, relative_threshold):
        ...
        cnts = sorted(
            cnts, key=Image_Utils.contour_bound_box_area, reverse=True)
        ...
        for c in cnts:
            ...
            Logger.get_logger().log_msg(Logger.DEBUG_FLAG_DEBUG, "Contour plot size fraction: " +
                                        str(contour_plot_size_fraction) + "; contour relative size fraction: " + str(contour_relative_size_fraction))
        ...
        image = cv2.bitwise_not(cv2.bitwise_and(
            image_temp, image_temp, mask=mask))
    ...
    def measure_plot_axis_width(image, is_x_axis):
        ...
        while (True):
            ...
            if (is_x_axis):
                lower_point_low = image[int(
                    plot_height/2)-lower_axis_width, lower_point]
                upper_point_low = image[int(
                    plot_height/2)-lower_axis_width, upper_point]

            else:
                lower_point_low = image[lower_point,
                                        int(plot_width/2)-lower_axis_width]
                upper_point_low = image[upper_point,
                                        int(plot_width/2)-lower_axis_width]
        ...
        while (True):
            ...
            if (is_x_axis):
                lower_point_high = image[int(
                    plot_height/2)+upper_axis_width, lower_point]
                upper_point_high = image[int(
                    plot_height/2)+upper_axis_width, upper_point]

            else:
                lower_point_high = image[lower_point,
                                         int(plot_width/2)+upper_axis_width]
                upper_point_high = image[upper_point,
                                         int(plot_width/2)+upper_axis_width]
